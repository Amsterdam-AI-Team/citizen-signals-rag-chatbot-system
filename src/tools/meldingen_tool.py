...
class MeldingenRetrieverTool:
    ...
    When provided with an address, it attempts to retrieve duplicate meldingen from the neighborhood.
    ...
    Attributes:
        ...
        ndex_storage_folder (str): path to folder where pre-computed indices are stored
    """
    ...
    def _load_meldingen(self):
        ...
        # self.meldingen_data = pd.read_csv(self.meldingen_path).head(1000)
        self.meldingen_data["location.geometrie.coordinates"] = self.meldingen_data["location.geometrie.coordinates"].apply(literal_return)
        self.meldingen_data = self.meldingen_data[~self.meldingen_data["location.geometrie.coordinates"].isna()]
        ...
        self.meldingen_data[["lat","lon"]] = pd.DataFrame(self.meldingen_data["location.geometrie.coordinates"].to_list(), index=self.meldingen_data.index)
        self.meldingen_geodata = gpd.GeoDataFrame(self.meldingen_data, geometry=gpd.points_from_xy(self.meldingen_data["lat"], self.meldingen_data["lon"]), crs='epsg:4326')


    def _get_persist_path(self):
        #TODO: Incorporate hashing to reindex on document update
        # print(f"{index_storage_folder}")
        return self.index_storage_folder / f"meldingen_{len(self.meldingen_data)}_{self.model_name.replace('/', '_')}.pkl"
    ...
    def _init_index(self):
        ...
        if self.index_persist_path.exists():
            ...
            with open(self.index_persist_path, "wb") as persist_file:
                pickle.dump({"documents": documents, "embeddings": self.corpus_embeddings}, persist_file)
    ...
    def filter_meldingen_embeddings(self, address):
        ...
        self.meldingen_geodata["distance"] = self.meldingen_geodata.to_crs("EPSG:28992").distance(poi)
        ...
        closest_per_category = self.meldingen_geodata.groupby("category.sub", group_keys=False).apply(lambda x: x.nsmallest(5, ["distance"]))
    ...
    def retrieve_meldingen(self, query, top_k=5, address=None):
        ...
        docs = meldingen_data.iloc[hit_ids].apply(
            lambda x: (
...
if __name__ == '__main__':
    # bash /home/azureuser/cloudfiles/code/blobfuse/blobfuse_meldingen.sh
    ...
    # Specify the address
    address = 'Admiralengracht 119, 1057ET'  # Replace with your desired address
    # print(address.split(', '))

    # Instantiate the BGTAgent class with the address
    retriever = MeldingenRetrieverTool(model_name, meldingen_path, index_folder)

    melding_vuilnis = "vuilnis ligt er al dagen"
    melding_zwervers = "Overlast door zwervers naast winkel. Heel onveilig situatie. Medewerkers en ons personeel zijn bang."
    melding_graffiti = "Graffiti op de brug, andere kant van waar net andere graffti verwjiderd was."

    print("===== With an address =====")
    pprint(list(enumerate(retriever.retrieve_meldingen(
        melding_vuilnis,
        address=address,
        top_k=10
    ))))
